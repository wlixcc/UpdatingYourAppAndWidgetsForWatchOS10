...
Abstract:
A helper extension to calculate animation progress and timing.
*/
...
public extension ClosedRange where Bound: BinaryFloatingPoint {
    ...
    /// - Parameters:
    ///   - percent: The percentage between the lower and the upper bound.
    ///   - clamped: Ignores values outside `0...1`, and defaults to `true`.
    /// - Returns: A value between the lower and the upper bound.
    ...
    ///   - clamped: Clamps the result between `0...1`, and defaults to `true`.
...
public extension BinaryFloatingPoint {
    /// Returns a value that smoothly ramps from 0 to 1, which useful for animation purposes.
    ///
    /// > Important: The value this method is called on must be a finite number.
    /// - Parameter clamped: Ignores values outside `0...1`, and defaults to `true`.
    /// - Returns: A value that eases from 0 to 1.
    ...
    /// `1` | `0`
    ///
    /// > Important: The value you use when calling this method needs to be a finite number.
    /// - Parameter clamped: Ignores values outside `0...1`, and defaults to `true`.
    /// - Returns: A value that eases from 0 to 1 and back to 0.
    ...
    ///
    /// If this value is 175 and you pass 50 for the `truncation` value, it returns 0.5 because it's 50% toward the next `truncation` value.
    ///
    /// > Important: The value you use when calling this method needs to be a finite number.
    /// - Parameter truncation: A value for truncating the starting value and determining the percentage.
    ...
    /// Maps this value, assumed to be a percentage from 0 to 1, to the corresponding value between the lower and upper bounds of `range`.
    ///
    /// If this value is 0.5 and you pass a range from 0 to 10, the result is 5 because it's 50% between 0 and 10.
    ///
    /// > Important: The value you use when calling this method needs to be a finite number.
    /// - Parameters:
    ///   - range: A closed range.
    ///   - clamped: Ignores values outside `0...1`, and defaults to `true`.
    /// - Returns: A value within the range of the value you use when calling this method, presumably a percentage from 0 to 1.
    func map(to range: ClosedRange<Self>, clamped: Bool = true) -> Self {
        assert(self.isFinite)
        return range.value(percent: self, clamped: clamped)
    }
    
    /// Maps a value from `originalRange` into a percentage that that the system uses to return an equivalent value in `newRange`.
    ///
    /// > Important: The value you use when calling this method needs to be a finite number.
    /// - Parameters:
    ///   - originalRange: A range to use when converting this value into a percentage.
    ///   - newRange: A range that determines the possible resulting value.
    ///   - clamped: Ignores percentages in either range that are outside `0...1`, and defaults to `true`.
